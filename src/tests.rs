use crate::{errors::{MathLibError, ParserError, QuickEvalError, QuickSolveError, SolveError}, parse, quick_eval, quick_solve, Value, Variable};

#[test]
fn easy_eval1() -> Result<(), MathLibError> {
    let res = quick_eval("3*3", vec![])?;
...
fn easy_eval4() -> Result<(), MathLibError> {
    let res = quick_eval("[[3, 4, 5], [1, 2, 3], [5, 6, 7]]", vec![])?;
...
fn easy_eval5() -> Result<(), MathLibError> {
    let res = quick_eval("[3, 3/4, 6]", vec![])?;
...
fn medium_eval1() -> Result<(), MathLibError> {
    let x = Variable::from_value("x", Value::Scalar(3.));
    let res = quick_eval("3x", vec![x])?;
...
fn medium_eval2() -> Result<(), MathLibError> {
    let a = Variable::from_value("A", Value::Vector(vec![3., 5., 8.]));
    let res = quick_eval("3A", vec![a])?;
...
fn medium_eval3() -> Result<(), MathLibError> {
    let a = Variable::from_value("A", Value::Vector(vec![3., 5., 8.]));
    let b = Variable::from_value("B", Value::Matrix(vec![vec![2., 0., 0.], vec![0., 2., 0.], vec![0., 0., 1.]]));
    let res = quick_eval("B*A", vec![a, b])?;
...
fn medium_eval4() -> Result<(), MathLibError> {
    let a = Variable::from_value("A", Value::Matrix(vec![vec![3., 5., 7.], vec![4., 8., 2.], vec![1., 9., 2.]]));
    let b = Variable::from_value("B", Value::Matrix(vec![vec![7., 9., 10.], vec![1., 55., 8.], vec![22., 9., 2.]]));
    let res = quick_eval("A*B", vec![a, b])?;
...
#[test]
fn medium_eval6() {
    let res = quick_eval("[[3, 0, 5], [2, 4, 5], [1, 2]]", vec![]);
...
#[test]
fn medium_eval9() {
    let res = quick_eval("[[3, 0,], [2, 4, 5], [1, 2]]", vec![]);
...
#[test]
fn medium_eval10() {
    let res = quick_eval("[[3, 0, 5], [2, 4], [1, 2,]]", vec![]);
...
fn medium_eval12() -> Result<(), MathLibError> {
    let res = quick_eval("[3, 3*3, -5]", vec![])?;
...
fn medium_eval13() -> Result<(), MathLibError> {
    let vars = vec![Variable::from_value("A_{3*6}", Value::Scalar(3.))];

    let res = quick_eval("A_{3*6}*3", vars)?;
...
fn medium_eval14() -> Result<(), MathLibError> {
    let res = quick_eval("[[1, 0], [0, 6], [0, 0]]*[3, 4, 5]", vec![])?;
...
fn medium_eval16() -> Result<(), MathLibError> {
    let res = quick_eval("3[4, 5, 6]", vec![])?;
...
fn medium_eval17() -> Result<(), MathLibError> {
    let res = quick_eval("3*[[2, 0, 0], [0, 1, 0], [0, 0, 5]]*[[1, 0, 0], [0, 1, 0], [0, 0, 1]]*[3, 4, 5]", vec![])?;
...
fn medium_eval18() -> Result<(), MathLibError> {
    let res = quick_eval("[sqrt(25), 2pi, 3]", vec![])?;
...
fn medium_eval19() -> Result<(), MathLibError> {
    let res = quick_eval("[0, 0.5, 0]#[-0.8, 0, 0.6]", vec![])?;
...
fn medium_eval21() -> Result<(), MathLibError> {
    ...
    let function_var = Variable::from_function("f", function, vec!["x"]);
    let a = Variable::from_value("A", Value::Vector(vec![3., 4., 5.]));

    let res = quick_eval("f([3, 4, 5])", vec![function_var, a])?;
...
fn calculus_eval1() -> Result<(), MathLibError> {
    let res = quick_eval("D(x^2, x, 3)", vec![])?;
...
fn calculus_eval2() -> Result<(), MathLibError> {
    let res = quick_eval("I(x^2, x, 0, 5)", vec![])?;
...
fn hard_eval1() -> Result<(), MathLibError> {
    let x = Variable::from_value("x", Value::Scalar(3.));
    let a = Variable::from_value("A", Value::Vector(vec![3., 2., 1.]));
    let b = Variable::from_value("B", Value::Matrix(vec![vec![2., 3., 4.], vec![5., 1., 7.], vec![2., 3., 6.]]));
    let res = quick_eval("(x*B*A)?1", vec![x, a, b])?;
...
fn medium_solve1() -> Result<(), MathLibError> {
    let res = quick_solve("3x^2+2x-1=0", vec![])?;
...
fn medium_solve4() -> Result<(), MathLibError> {
    ...
    let mut res = quick_solve("f(x)=0", vec![function_var])?;
...
fn calculus_solve1() -> Result<(), MathLibError> {
    let res = quick_solve("D(3x^2+2x-1, x, k)=0", vec![])?;
...
fn hard_solve2() -> Result<(), MathLibError> {
    let res = quick_solve("x*[3, 4, 5]=[6, 8, 10]", vec![]);
