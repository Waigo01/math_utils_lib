...
#[test]
fn simple_eval() -> Result<(), MathLibError> {
...
fn medium_eval4() -> Result<(), MathLibError> {
    ...
    let res = quick_eval("A*B".to_string(), Some(vec![a, b]))?;
...
fn hard_eval1() -> Result<(), MathLibError> {
    ...
    let res = quick_eval("x*B*A?1".to_string(), Some(vec![x, a, b]))?;
...
#[test]
fn easy_test1() -> Result<(), MathLibError> {
